...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_dinosaurs
    ...
    animals.each do |animal|
      dinosaurs << animal if animal[-6,6] == "saurus"
    end
  ...
  def test_pick_floats
    ...
    floats = []
    numbers.each do |number|
      floats << number if number.to_s.include?(".") == true
    end
  ...
  def test_pick_arrays
    ...
    elements.each do |element|
      arrays << element if element.kind_of?(Array) == true
    end
  ...
  def test_pick_hashes
    ...
    hashes = []
    elements.each do |element|
      hashes << element if element.kind_of?(Hash) == true
    end
